...
exports.isNewDeviceOrLocation = isNewDeviceOrLocation;
...
async function checkAccountLockout(userId) {
    ...
    const recentFailures = await prisma_1.default.loginHistory.count({
        where: {
            userId,
            success: false,
    ...
    if (recentFailures >= MAX_FAILED_ATTEMPTS) {
        ...
        const lastFailure = await prisma_1.default.loginHistory.findFirst({
            where: {
                userId,
                success: false,
                createdAt: { gte: lockoutWindow }
...
async function isNewDeviceOrLocation(userId, req) {
    ...
    const existingFromIp = await prisma_1.default.loginHistory.findFirst({
        where: {
            userId,
            ipAddress,
            success: true
    ...
    const existingDevice = await prisma_1.default.loginHistory.findFirst({
        where: {
            userId,
            device,
            browser,
